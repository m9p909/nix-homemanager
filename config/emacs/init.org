...
#+begin_src emacs-lisp
    ...
    (defun start/tangle-config ()
      ...
      (interactive)
      (let ((config-file (expand-file-name "init.org" user-emacs-directory))
            (org-confirm-babel-evaluate nil))
        (with-current-buffer (find-file-noselect config-file)
          (org-babel-tangle))))

    (defun start/reload-config ()
      "Tangle init.org and reload Emacs config."
      (interactive)
      (start/tangle-config)
      (load-file (expand-file-name "init.el" user-emacs-directory)))

    (defun start/eval-current-form ()
      "Evaluate the current Org source block or CIDER sexp."
      (interactive)
      (cond
       ((derived-mode-p 'org-mode)
        (if (org-in-src-block-p)
            (org-babel-execute-src-block)
          (user-error "Point is not inside an Org source block")))
       ((derived-mode-p 'clojure-mode 'cider-repl-mode)
        (cider-eval-last-sexp))
       (t
        (user-error "No evaluation command configured for %s" major-mode))))
    ...
    (use-package general
      ...
      (start/leader-keys
        ...
        "e e" '(start/eval-current-form :wk "Eval form/sexp")
      ...
      (start/leader-keys
        "e c" '(:ignore t :wk "CIDER")
        "e c d" '(cider-eval-defun-at-point :wk "Eval defun")
        "e c s" '(cider-eval-last-sexp :wk "Eval last sexp")
        "e c r" '(cider-eval-region :wk "Eval region")
        "e c b" '(cider-eval-buffer :wk "Eval buffer"))
      ...
      (start/leader-keys
       "a" '(:ignore t :wk "AI")
        ...
        (dotimes (i 9)
          (let ((n (1+ i))
                (key (number-to-string (1+ i))))
            (eval `(start/leader-keys
                    ,key '((lambda () (interactive) (tab-bar-select-tab ,n))
                            :wk ,(format "Tab %d" n))))))
...
*** Gptel
Use gptel as the LLM client.
#+begin_src emacs-lisp
        (use-package gptel
          :config
          (setq gptel-default-mode 'markdown-mode)
         (setq gptel-backend (gptel-make-openai "MyOAICOMPATIBLE"          ; Name shown in the gptel menu
                            :host (or (getenv "OPENAI_BASE_URL") "idk")                 ; API hostname
                            :protocol "https"                        ; or "http" for local servers
                            :endpoint "/v1/chat/completions"          ; The chat completions path
                            :stream t                                ; Enable streaming responses
                            :key (getenv "OPENAI_API_KEY")         ; API key from env va
...
#+begin_src emacs-lisp
    (setq treesit-language-source-alist
          '((bash "https://github.com/tree-sitter/tree-sitter-bash")
            ...
            (ino "https://github.com/tree-sitter-grammars/tree-sitter-ino")
...
(use-package eat
  ...
  :bind (:map eat-mode-map
              ("C-c e" . (lambda () (interactive) (eat-term-send-string  eat-terminal "\e")))))
#+end_src
...
(use-package evil-ghostel
  ...
  :init (setq-default evil-ghostel-escape 'terminal)
  :bind (
         :map evil-ghostel-mode-map
          ("C-c C-e" . evil-normal-state)))
