---
description: Executes an already-defined plan, iterating until approved.
---
...
Read the plan from context or ask to paste it if not provided.
...
If any of these are missing, ask for a plan update before proceeding.

---

## Execution Loop

Repeat the following until approved:

1. **Implement**: Use the Task tool to spawn an implementer agent with the full plan and the following prompt:
...
2. **Review**: Use the Task tool to spawn a reviewer agent with the following prompt:
   ...
   >
   > **Code Quality** (enforce AGENTS.md standards)
...
3. Present the reviewer's verdict and a summary of test results. Ask for approval.

4. If changes are requested, incorporate the feedback and the reviewer's notes, then repeat from step 1.
